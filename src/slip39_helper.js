...
class CryptoHelper {
  ...
  static roundFunction(round, passphrase, exp, salt, secret) {
  ...
  static createDigest(randomData, sharedSecret) {
...
class ShamirHelper {
  ...
  static splitSecret(threshold, shareCount, sharedSecret) {
    if (threshold <= 0) {
      throw Error(
  ...
  static recoverSecret(threshold, shares) {
    // If the threshold is 1, then the digest of the shared secret is not used.
    if (threshold === 1) {
  ...
  static interpolate(shares, x) {
    ...
    shares.forEach((v, k) => {
      ...
      // FIXME: -18 % 255 = 237. IT should be 237 and not -18 as it's
...
class MnemonicHelper {
  ...
  static decodeMnemonics(mnemonics) {
  ...
  static mnemonicFromIndices(indices) {
  ...
  static mnemonicToIndices(mnemonic) {
...
class Slip39Helper {
  ...
  static slip39DecodeHex = function (arr) {
    let str = [];
  ...
  static toByteArray(arr, hexString) {
  ...
  static BIGINT_WORD_BITS = BigInt(8);
  ...
  static encodeBigInt(number, paddedLength = 0) {
    let num = number;
